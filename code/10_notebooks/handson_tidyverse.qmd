...
```{r}
library(tidyverse)
library(WDI) # World Development Indicators package.
...
```{r}
# Take a look to the `country` variable.
...
fertility |>
  filter(iso3c %in% oecd) |> # get OECD country subset
  group_by(country) |> # Group dataset by country
...
unpaid_male |> 
  group_by(country) |>
  #sort('year') |>
  top_n(n=1, year) # Get the first n rows of the dataset.
...
df_new <-
  fertility |>
  left_join(unpaid_male, by = c('country', 'year'))
```
